...
describe("@soralabsoss/generator", () => {
  it("normalizes partial brand input without mutating defaults", () => {
    ...
    expect(config.statusColors.warning).toBe(
      initialConfig.statusColors.warning
    );
  ...
  it("generates deterministic light and dark token sets", () => {
    ...
    expect(light["--typography-font-family-body"]).toContain(
      initialConfig.primaryFont
    );
  ...
  it("generates CLI-ready artifacts from one-shot inputs", () => {
    const theme = generateTheme(
      ...
      { colorSpace: "hex", formats: ["css", "shadcn"] }
    );

    expect(theme.config.primaryColor).toBe("#ffcdcf");
    expect(theme.artifacts.map((artifact) => artifact.format)).toEqual([
      "css",
      "shadcn",
    ]);
