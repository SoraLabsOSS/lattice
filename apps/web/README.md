...
```bash
bun run --filter @sora-lattice/web dev       # astro dev (default http://localhost:4321)
bun run --filter @sora-lattice/web build     # astro build → dist/
bun run --filter @sora-lattice/web preview   # serve dist/
